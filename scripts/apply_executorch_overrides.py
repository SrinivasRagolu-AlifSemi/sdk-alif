...
def patch_torch_versions(executorch_dir: Path) -> bool:
    ...
    The original versions (2.11.0.dev20251222) are no longer available,
    so we update to newer compatible versions.
    ...
    if torch_pin_file.exists():
        ...
        content = content.replace('TORCH_VERSION = "2.11.0"', 'TORCH_VERSION = "2.12.0"')
        content = content.replace('NIGHTLY_VERSION = "dev20251222"', 'NIGHTLY_VERSION = "dev20260223"')

        if content != original_content:
            with open(torch_pin_file, 'w') as f:
                f.write(content)
            print(f"  ✓ Updated torch_pin.py: TORCH_VERSION=2.12.0, NIGHTLY_VERSION=dev20260223")
        else:
            print(f"  ✓ torch_pin.py already up to date")
    else:
        print(f"  Warning: torch_pin.py not found")

    # Patch install_requirements.py
    ...
    if install_req_file.exists():
        ...
        original_content = content
        content = content.replace('torchvision==0.25.0.{NIGHTLY_VERSION}', 'torchvision==0.26.0.{NIGHTLY_VERSION}')
        content = content.replace('torchaudio==2.10.0.{NIGHTLY_VERSION}', 'torchaudio==2.11.0.{NIGHTLY_VERSION}')

        if content != original_content:
            with open(install_req_file, 'w') as f:
                f.write(content)
            print(f"  ✓ Updated install_requirements.py: torchvision=0.26.0, torchaudio=2.11.0")
        else:
            print(f"  ✓ install_requirements.py already up to date")
    else:
